...
export function normalizeJournalSizeKey(value?: string): string | null {
  if (!value) return null;
...
export function normalizePricingSizeKey(
  ...
  if (productType === "journal") {
    return normalizeJournalSizeKey(variant.size) ?? normalizeJournalSizeKey(variant.name);
  }
